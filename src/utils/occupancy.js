...
export function buildMonthData(bookings, year, month) {
  ...
  for (const b of bookings) {
    ...
    for (const dateStr of Object.keys(occupancyMap)) {
...
export function buildMonthStats(bookings, year, month) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (count > peakOccupancy) peakOccupancy = count;
  }

  // Per-booking stats for bookings overlapping this month
  for (const b of bookings) {
    if (b.status === "cancelled") continue;
    if (b.checkIn > monthEnd || b.checkOut <= monthStart) continue;
